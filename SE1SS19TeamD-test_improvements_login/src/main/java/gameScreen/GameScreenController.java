...
public class GameScreenController {
	 ...
	 * 
	 * @param own
	 *            the scene this is the controller of
	 */
	...
	/**
	 * load all elements for the gamescreen set the actions for the opacity change
	 * of the map and unitsdisplay
	 */
	@FXML
	public void initialize() {
		...
		if (!test) {
			Timeline timeline = new Timeline(new KeyFrame(Duration.seconds(20), t -> reloadField()));
	...
	public void reloadField() {
		...
		if (Model.getApp().getCurrentPlayer().getGame().getGameField() != null) {
			GameField tmp = GamefieldGenerator
					.generateGameField(Model.getApp().getCurrentPlayer().getGame().getGameField().getFields());
