...
public class WebSocketRequests {
    ...
    public WebSocketRequests(URI uri, Model model) {
        ...
        if (this.uri.toString().startsWith(BS_WS_URI + CHAT_WS)
            ...
            this.handler = new WebSocketSystemHandler(model);
        } else if (this.uri.toString().startsWith(BS_WS_URI + GAME_WS)) {
            this.handler = new WebSocketGameHandler(model);
        }
    }

    /**
     * Creates a connection between the ClientEndpoint and the ServerEndpoint by
     * doing a "handshake".
     */
    public void connect() {
        try {
            WebSocketContainer container = ContainerProvider.getWebSocketContainer();
            model.getWebSocketConfigurator().setToMine();
     ...
     * ClientEndpoint is successfully established after a handshake. It also starts
     * a schedule to send a "noop" to the ServerEndpoint to hold the connection.
     *
    ...
    /**
     * This method is called when the ServerEndpoint has sent a message to the
     * ClientEndpoint. The message will be handled by the specific WebSocketHandler.
     *
     * @param message is a JSONObject as a String that we receive from the
     *                ServerEndpoint.
     */
    @OnMessage
    ...
    /**
     * This method is called once the connection is being closed. It also tries to
     * reconnect the ClientEndpoint to the ServerEndpoint in case of an abnormal
     * close reason. The code "1000" stands for a normal closure.
     *
    ...
    /**
     * This method is called when an Exception is being thrown by any method
     * annotated with @OnOpen, @OnMessage and @OnClose. Since the methods of this
     * class won't throw an Exception, this method will only be called when the
     * methods of the ServerEndpoint throw an Exception.
     *
     * @param session is the session we received from the server.
    ...
    /**
     * Checks if the message is a private message or an all-chat message. Uses the
     * specific method afterwards.
     *
    ...
    public void sendChatMessage(String channel, String from, String msg) {
        ...
        JSONObject jsonObject = new JSONObject().put("channel", channel).put("from", from).put("message", msg);
    ...
    public void sendChatMessage(String channel, String from, String msg, String to) {
        ...
        JSONObject jsonObject = new JSONObject().put("channel", channel).put("from", from).put("message", msg).put("to",
                to);
    ...
    /**
     * This method checks if the connection with the ServerEndpoint is still
     * established.
     *
     * @return returns true if the session is not closed yet and false if it is
     * closed.
     */
     ...
     *
     * @param message contains all necessary information about the message and the
     *                sender/receiver.
     */
    ...
    /**
     * Sends a private message to a player that is in the same game as the current
     * player.
     *
     * @param message contains all necessary information about the message and the
     *                sender/receiver.
    ...
    public void sendPrivateChatGameMessage(ChatMessage message) {
        if (!this.uri.toString().startsWith(BS_WS_URI + GAME_WS)) {
            ...
            json.put("messageType", "chat").put("channel", message.getChannel())
                    .put("from", message.getSender().getName()).put("to", message.getReceiver().getName())
