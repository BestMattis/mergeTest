...
public class LoginRegisterTestUtils {
	...
	public static void createGameForOfflineTest(ApplicationTest test) {
		...
		JSONArray initialGames = new JSONArray().put(new JSONObject().put("joinedPlayer", 1).put("name", "My Game")
	...
	private static Label performWhileNotPresent(ApplicationTest test) {
		...
		return (Label) test.lookup("#gameName").queryAllAs(Label.class).stream()
				.filter(x -> x.getText().equals("My Game")).findFirst().get();
	}
}
